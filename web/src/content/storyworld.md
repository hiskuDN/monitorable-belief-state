...
*Epistemic status: small synthetic experiments (~7M-parameter models), now on a language surface instead of a grid. Third in a series. It includes a self-correction: a single-seed run that briefly fooled me. Early, sharing for discussion.*

The previous two posts ran in symbolic worlds: [a gridworld where a model tracks its own position](https://hisku.substack.com/p/monitorability-as-a-training-objective), then [a symbolic task where a model hides a running secret](https://hisku.substack.com/p/catching-a-model-that-hides-a-secret). Both times a predict-ahead training objective kept the model's belief state legible to a simple linear probe, where a vanilla model let it go dark. The obvious objection writes itself: real models work on *language*, not grids and symbols. So this time I moved the same test onto text.
...
A short narrative moves a ball around a few numbered shelves. "The ball starts on shelf 2." Then a run of sentences nudges it by relative steps: "Alice moves the ball up one shelf," "Bob moves the ball down one." A second object, a book, is shuffled around at the same time as a distraction. At the end comes the question: "Where is the ball?" Both models answer it correctly.
...
On the task (four shelves, six moves, one distractor), both a plain GPT and a NextLat model that predicts a single step ahead answer the final question perfectly, on every seed. But watch the monitor.
...
Two things, because the path here was not clean.

**A single seed fooled me.** My first run used one random seed, and the plain model happened to score badly on the task itself. I briefly wrote down the wrong conclusion: that *carrying is necessary to solve*. Three seeds killed that. The plain model solves the task perfectly and still defers. The finding is carry-versus-defer, not carry-or-fail. Both models know the answer; only one holds it where you can see it, and that is the whole point.
...
Push the story harder (start saying "it" instead of naming the object, and add a second distractor) and the gap frays but survives: the predictive model still carries the belief on two of three seeds, the plain model defers on all three. Push further, into the version no small model can track at all, and everyone goes dark, the carrier included. So this is not a law; it is an effect with a ceiling. It holds while the task sits inside what a small model can actually track, and thins as the language surface gets harder. I would rather show you the fraying than a single cherry-picked story.
