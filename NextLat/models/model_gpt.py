...
class Transformer(DocumentRelativePositions, FusedCrossEntropyLoss, nn.Module):
    ...
    def forward(
        ...
        return_all_layers: bool = False,  # Per-layer residual-stream states for probing
    ) -> torch.Tensor:
        ...
        # Per-layer states for probing (Experiment 0): each post-block state plus
        # the final normed state as the last entry. Backward-compatible (off by
        # default); returns logits as the first element.
        if return_all_layers:
            all_layers.append(x)
            return self.lm_head(x), all_layers
