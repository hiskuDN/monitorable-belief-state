...
class NextLatTransformer(DocumentRelativePositions, FusedCrossEntropyLoss, nn.Module):
    ...
    def forward(
        ...
        # Per-layer residual-stream states for probing (Experiment 0).
        # Returns each post-block state plus the final normed state as the last
        # entry; backward-compatible (off by default).
        if return_all_layers:
            all_layers.append(text_embd)
            return self.lm_head(text_embd), all_layers
