...
class MTPTransformer(DocumentRelativePositions, FusedCrossEntropyLoss, nn.Module):
    ...
    def forward(
        ...
        return_all_layers: bool = False,  # per-trunk-layer states for probing
    ) -> torch.Tensor:
        ...
        for block in self.shared_trunk.blocks:
            ...
            if return_all_layers:
                all_layers.append(x)
        # Per-trunk-layer states for probing (the comparable backbone residual
        # stream). The MTP trunk has no final norm, so the last entry is raw x.
        if return_all_layers:
            all_layers.append(x)
